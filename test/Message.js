...
import { Message as Message_Message } from "../lib/Message";
...
describe('Message', function () {
    it('should accept an options object with headers and body', function () {
        ...
        ext_expect(message.body, 'to equal', 'abc');
        ext_expect(message.headers.getAll('received'), 'to equal', ['foo', 'bar']);
        ext_expect(message.headers.getAll('subject'), 'to equal', ['hey']);
        ext_expect(message.toString(), 'to equal', 'Received: foo\r\nReceived: bar\r\nSubject: hey\r\n\r\nabc');
    });

    it('should parse the headers from the input', function () {
        var message = new Message_Message('From: thisguy@example.com\r\nTo: thisotherguy@example.com');
        ext_expect(message.headers.getNames(), 'to equal', ['from', 'to']);
    });
    ...
    it('should support reading a Buffer instance with iso-8859-1 chars', function () {
        ...
        ext_expect(new Message_Message(buffer).headers.get('From'), 'to equal', 'øfoobarquux');
    });

    it('should handle folded lines when parsing', function () {
        var message = new Message_Message('Subject: abc\r\n def');
        ext_expect(message.headers.get('subject'), 'to equal', 'abc def');
    });

    it('should handle folded lines with just CR when parsing', function () {
        var message = new Message_Message('Subject: abc\r def');
        ext_expect(message.headers.get('subject'), 'to equal', 'abc def');
    });
    ...
    it('should keep the buffer as a Buffer when the message is provided as a Buffer', function () {
        ext_expect(new Message_Message(Buffer.concat([
            new Buffer(
                'From: foo@bar\r\n' +
                '\r\n',
                'utf-8'
    ...
    it('should detect the end of the headers properly when separated by CRCR', function () {
        ...
        ext_expect(new Message_Message(new Buffer('From: foo@bar\r\r', 'utf-8')), 'not to have property', 'body');
    });
    ...
    it('should detect the end of the headers properly when separated by LFLF', function () {
        ...
        ext_expect(new Message_Message(new Buffer('From: foo@bar\n\n', 'utf-8')), 'not to have property', 'body');
    });

    it('should not read past CRLFCRLF when parsing', function () {
        var message = new Message_Message('Subject: abc\r\n\r\nFrom: me');
        ext_expect(message.headers.get('from'), 'to be', undefined);
    });

    it('should not read past CRCR when parsing', function () {
        var message = new Message_Message('Subject: abc\r\rFrom: me');
        ext_expect(message.headers.get('from'), 'to be', undefined);
    });
    ...
    it('should produce an empty string when handed an empty buffer', function () {
        ext_expect(new Message_Message(new Buffer(0)).toString(), 'to equal', '');
    });

    it('should parse Real Life message from Apple Mail', function () {
        var message = new Message_Message(new Buffer([
        ...
        ext_expect(message.headers.getNames(), 'to equal', ['content-type', 'content-transfer-encoding', 'subject', 'from', 'message-id', 'date', 'to', 'mime-version']);
        ext_expect(message.headers.get('Content-Type'), 'to equal', 'multipart/mixed;\tboundary=Apple-Mail-589ECA5D-7F89-4C39-B7B7-7FD03E6333CD');
        ext_expect(message.headers.get('content-transfer-encoding'), 'to equal', '7bit');
        ext_expect(message.headers.get('Subject'), 'to equal', 'Foobar 123');

        ext_expect(message.toString(), 'to contain', 'multipart\/mixed;\r\n\tboundary=Apple-Mail-589ECA5D-7F89-4C39-B7B7-7FD03E6333CD');
    });

    it('should preserve repeated headers', function () {
        var message = new Message_Message('Received: foo\r\nReceived: bar\r\n');
    ...
    it('should preserve text after CRLFCRLF as-is', function () {
        var message = new Message_Message('foo: bar\r\n\r\nthis is the:body');

        ext_expect(message.toString(), 'to equal', 'Foo: bar\r\n\r\nthis is the:body');
    });

    it('should preserve text after CRCR as-is', function () {
        var message = new Message_Message('foo: bar\r\rthis is the:body');
    ...
    it('should read an iso-8859-1 body after LFCRLFCR into a buffer and turn it into REPLACEMENT CHARACTER U+FFFD when serializing as text', function () {
        var message = new Message_Message(Buffer.concat([new Buffer('foo: bar\n\r\n\rthis is the:body', 'utf-8'), new Buffer([0xf8, 0xe6])]));
        ext_expect(message.body, 'to equal', Buffer.concat([new Buffer('this is the:body', 'utf-8'), new Buffer([0xf8, 0xe6])]));
        ext_expect(message.toString(), 'to equal', 'Foo: bar\r\n\r\nthis is the:body\ufffd\ufffd');
    });

    it('should treat a body passed as a Buffer as the unchunked body', function () {
        var message = new Message_Message({ body: new Buffer([0xff]) });
        ext_expect(message._unchunkedBody, 'to equal', new Buffer([0xff]));
    });

    describe('#hasEmptyBody', function () {
        it('should consider a zero length Buffer to be an empty body', function () {
            ext_expect(new Message_Message({body: new Buffer([])}).hasEmptyBody(), 'to be true');
        });

        it('should consider a non-zero length Buffer not to be an empty body', function () {
            ext_expect(new Message_Message({body: new Buffer([123])}).hasEmptyBody(), 'to be false');
        });

        it('should consider the empty string to be an empty body', function () {
            ext_expect(new Message_Message({body: ''}).hasEmptyBody(), 'to be true');
        });

        it('should consider a non-empty string not to be an empty body', function () {
            ext_expect(new Message_Message({body: 'foo'}).hasEmptyBody(), 'to be false');
        });

        it('should consider undefined to be an empty body', function () {
            ext_expect(new Message_Message({body: undefined}).hasEmptyBody(), 'to be true');
        });

        it('should consider an absent body to be empty', function () {
            ext_expect(new Message_Message().hasEmptyBody(), 'to be true');
        });

        it('should consider an empty Object to be a non-empty body', function () {
            ext_expect(new Message_Message({body: {}}).hasEmptyBody(), 'to be false');
        });
    });

    describe('with a JSON body', function () {
        it('should provide a parsed body (and default to utf-8)', function () {
            ext_expect(new Message_Message(new Buffer('Content-Type: application/json\n\n{"foo":"æøå"}', 'utf-8')).body, 'to equal', {
                foo: 'æøå'
            });
        });

        it('should support a Content-Type that ends with +json', function () {
            ext_expect(new Message_Message(new Buffer('Content-Type: foobar+json; charset=UTF-8\n\n{"foo":"æøå"}', 'utf-8')).body, 'to equal', {
                foo: 'æøå'
            });
        });

        it('should reserialize the body as utf-8', function () {
            var message = new Message_Message(new Buffer('Content-Type: application/json\n\n{"foo":"æøå"}', 'utf-8'));
            message.body = { foo: '☺' };
            ext_expect(message.rawBody, 'to equal', new Buffer('{"foo":"☺"}', 'utf-8'));
        });
        ...
        describe('with an application/json body that does not parse as JSON', function () {
            ...
            it('should leave unchunkedBody as-is', function () {
                ext_expect(message.unchunkedBody, 'to equal', '!!=!=');
            });

            it('should provide body as a string', function () {
                ext_expect(message.body, 'to equal', '!!=!=');
            });
        });
    });
    ...
    describe('with parts passed to the constructor', function () {
        it('should accept both messy.Message instances and valid constructor arguments ', function () {
            var message = new Message_Message({
                ...
                parts: [
                    new Message_Message('Content-Type: text/html\r\n\r\n<h1>Hello, world!</h1>'),
                    { headers: 'Content-Type: text/plain', body: 'Hello, world!' },
                    'Content-Type: text/foo\r\n\r\nhey'
                ]
            });
            ext_expect(message.parts.length, 'to equal', 3);
            ext_expect(
                message.toString(),
                'to equal',
                'Content-Type: multipart/mixed\r\n' +
        ...
        it('should pick up the boundary with boundary', function () {
            ...
            ext_expect(message.parts.length, 'to equal', 1);
            ext_expect(
                message.toString(),
                'to equal',
        ...
        it('should allow passing a single part', function () {
            var message = new Message_Message({
                headers: 'Content-Type: multipart/mixed; boundary=foo',
                parts: new Message_Message('Content-Type: text/html\r\n\r\n<h1>Hello, world!</h1>')
            });
            ext_expect(message.parts.length, 'to equal', 1);
            ext_expect(
                message.toString(),
    ...
    describe('with a multipart body', function () {
        ...
        it('should decode the multipart parts when the body is passed as a Buffer', function () {
            var message = new Message_Message(new Buffer(src, 'utf-8'));

            ext_expect(message.toString(), 'to equal', src);

            ext_expect(message.parts, 'to equal', [
                new Message_Message(new Buffer('Content-Disposition: form-data; name="recipient"\r\n\r\nandreas@one.com', 'utf-8')),
                new Message_Message(new Buffer('Content-Disposition: form-data; name="Name "\r\n\r\nThe name', 'utf-8')),
                new Message_Message(new Buffer('Content-Disposition: form-data; name="email"\r\n\r\nthe@email.com', 'utf-8')),
                new Message_Message(new Buffer('Content-Disposition: form-data; name="Message "\r\n\r\nThe message', 'utf-8'))
            ]);
        });

        it('should decode the multipart parts when the body is passed as a string', function () {
            var message = new Message_Message(src);

            ext_expect(message.parts, 'to equal', [
                new Message_Message('Content-Disposition: form-data; name="recipient"\r\n\r\nandreas@one.com'),
                new Message_Message('Content-Disposition: form-data; name="Name "\r\n\r\nThe name'),
                new Message_Message('Content-Disposition: form-data; name="email"\r\n\r\nthe@email.com'),
                new Message_Message('Content-Disposition: form-data; name="Message "\r\n\r\nThe message')
            ]);
        });
        ...
        it('should reparse the parts if the body of the containing Message is updated', function () {
            ...
            message.parts.splice(1, 3);
            message.body = src;
            ext_expect(message.parts, 'to have length', 4);
        });
        ...
        it('should recompute the body if the parts are updated, binary mode', function () {
            ...
            ext_expect(body, 'to be a', Buffer);
            ext_expect(body.toString('utf-8'), 'to equal',
                '----------------------------231099812216460892104111\r\n' +
                'Content-Disposition: form-data; name="recipient"\r\n' +
                '\r\n' +
                '\x00\r\n' +
    ...
    describe('#unchunkedBody', function () {
        it('should decode Transfer-Encoding:chunked when the body is provided as a string', function () {
            ext_expect(new Message_Message(
        ...
        describe('when accessed as a setter', function () {
            it('should update rawBody and body', function () {
                ...
                ext_expect(message.rawBody, 'to equal', new Buffer('6\r\nfoobar\r\n0\r\n\r\n', 'utf-8'));
                ext_expect(message.body, 'to equal', 'foobar');
            });
        });
    ...
    describe('#body', function () {
        ...
        it('should decode a base64 body to a string when the Content-Transfer-Encoding is base64 and the Content-Type is textual and the body is stored as a Buffer', function () {
            ext_expect(new Message_Message(new Buffer(
                'Content-Type: text/plain; charset=UTF-8\r\n' +
                'Content-Transfer-Encoding: base64\r\n' +
                '\r\n' +
                'Zm9v\r\n')
        ...
        it.skipIf(!ext_zlib.gunzipSync, 'should decode Content-Encoding:gzip', function () {
            ext_expect(new Message_Message(Buffer.concat([
                new Buffer(
                    'Content-Type: text/plain; charset=UTF-8\r\n' +
        ...
        it('should decode application/x-www-form-urlencoded as text', function () {
            ...
            ext_expect(new Message_Message(src).body, 'to equal', 'foo=bar&data=%5B%22foo.txt%22%5D');
        });
    });
    ...
    describe('#rawBody', function () {
        it('should be populated when instantiating a Message from a string', function () {
            var rawBody = 'Foo: bar\r\n\r\nquux';
            ext_expect(new Message_Message(rawBody).rawBody, 'to equal', 'quux');
        });

        it('should be populated when instantiating a Message from a Buffer', function () {
            var rawBody = new Buffer('Foo: bar\r\n\r\nquux', 'utf-8');
            ext_expect(new Message_Message(rawBody).rawBody, 'to equal', new Buffer('quux', 'utf-8'));
        });
        ...
        it('should be recomputed from the body if updated, with Content-Transfer-Encoding', function () {
            ...
            ext_expect(message.rawBody, 'to equal', 'Zm9vYmFy');
            ext_expect(message.body, 'to equal', 'foobar');
            message.body = 'quux';
            ext_expect(message.rawBody, 'to equal', 'cXV1eA==');
        });
        ...
        it('should be recomputed from the body if updated, with quoted-printable and a charset', function () {
            ...
            ext_expect(message.rawBody, 'to equal', 'F=E6');
            ext_expect(message.body, 'to equal', 'Fæ');
            message.body = 'øh';
            ext_expect(message.rawBody, 'to equal', '=F8h');
        });
        ...
        it('should be recomputed from the body if updated, with transfer-encoding: chunked', function () {
            ...
            ext_expect(message.rawBody, 'to equal', '4\r\nWiki\r\n0\r\n\r\n');
            ext_expect(message.body, 'to equal', 'Wiki');
            message.body = 'sarbarbarbab';
            ext_expect(message.rawBody, 'to equal', new Buffer('c\r\nsarbarbarbab\r\n0\r\n\r\n'));
            ext_expect(message.body, 'to equal', 'sarbarbarbab');
        });

        it.skipIf(!ext_zlib.gzipSync, 'should be recomputed from the body if updated, with Content-Encoding:gzip', function () {
            var message = new Message_Message(Buffer.concat([
                new Buffer(
                    'Content-Type: text/plain; charset=UTF-8\r\n' +
                    'Content-Encoding: gzip\r\n' +
                    '\r\n', 'ascii'
            ...
            ext_expect(message.body, 'to equal', 'foobar');
            message.body = 'barfoo';
            ext_expect(message.rawBody, 'to equal', new Buffer([0x1f, 0x8b, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03, 0x4b, 0x4a, 0x2c, 0x4a, 0xcb, 0xcf, 0x07, 0x00, 0x2b, 0x85, 0xa8, 0xe2, 0x06, 0x00, 0x00, 0x00]));
            ext_expect(message.body, 'to equal', 'barfoo');
        });
    });
    ...
    describe('#fileName', function () {
        describe('when invoked as a getter', function () {
            it('should decode the Content-Disposition filename', function () {
                ...
                ext_expect(message.fileName, 'to equal', 'ræally screwed up long attachment filename with smileys☺ and ☺and ¡Hola, señor! and foreign weirdnessםולש ןב ילטפנ in it.☺');
            });
            ...
            it('should not fall back to the name property of the Content-Type header when the Content-Disposition header has no filename parameter', function () {
                ...
                ext_expect(message.fileName, 'to equal', undefined);
            });
        ...
        describe('when invoked as a setter', function () {
            it('should support a fileName setter which updates the Content-Disposition filename with the rfc2231 encoded representation', function () {
                var message = new Message_Message({body: 'bar'});
            ...
            it('should not update the name property of the Content-Type header, even if available', function () {
                var message = new Message_Message({headers: {'Content-Type': 'image/png'}});
    ...
    describe('#toJSON', function () {
        it('should include headers if any are present', function () {
            ext_expect(new Message_Message('Foo: bar').toJSON(), 'to equal', {
                headers: {
                    Foo: 'bar'
                }
            });
        ...
        it('should include the parsed representation of the body if it has been touched', function () {
            var message = new Message_Message('Content-type: application/json\r\n\r\n{"foo":"bar"}');
            message.body.blah = 123;
            ext_expect(message.toJSON(), 'to equal', {
                headers: {
    ...
    describe('with a JSON body provided as an object despite a non-JSON Content-Type', function () {
        ...
        it('should stringify correctly', function () {
            ext_expect(message.toString(), 'to equal',
                'Content-Type: application/octet-stream\r\n' +
                '\r\n' +
                '{"foo":123}'
            );
        ...
        it('should have the correct unchunkedBody', function () {
            ext_expect(message.unchunkedBody.toString('utf-8'), 'to equal', '{"foo":123}');
        });
    });
});
