...
describe('HttpResponse', function () {
    ...
    it('should accept the status line as an option to the constructor', function () {
        expect(new libHttpResponse_HttpResponsejs({statusLine: 'HTTP/1.1 200 OK'}), 'to have properties', {
            protocol: 'HTTP/1.1',
            statusCode: 200,
            statusMessage: 'OK'
        });
    ...
    it('should accept a number and interpret it as the status code', function () {
        expect(new libHttpResponse_HttpResponsejs(404), 'to have properties', {
            statusCode: 404
        });
    });

    it('should parse a partial status line', function () {
        expect(new libHttpResponse_HttpResponsejs('HTTP/1.1 200'), 'to have properties', {
            protocol: 'HTTP/1.1',
            statusCode: 200
        });
    });
    ...
    it('should only include CRLFCRLF when there are no headers', function () {
        expect(new libHttpResponse_HttpResponsejs({
    ...
    it('should make the protocol version available as a getter', function () {
        expect(new libHttpResponse_HttpResponsejs('HTTP/1.1 200 OK').protocolVersion, 'to equal', '1.1');
    });

    it('should make the protocol name available as a getter', function () {
        expect(new libHttpResponse_HttpResponsejs('HTTP/1.0 200 OK').protocolName, 'to equal', 'HTTP');
    });
    ...
    describe('#toJSON', function () {
        it('should include the messy.Message properties and put the statusLine properties at the top level', function () {
            expect(new libHttpResponse_HttpResponsejs('HTTP/1.1 200 OK\r\nFoo: bar\r\n\r\nblabla').toJSON(), 'to equal', {
                protocolName: 'HTTP',
                protocolVersion: '1.1',
                statusCode: 200,
                statusMessage: 'OK',
