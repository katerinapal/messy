...
import { StatusLine as libStatusLine_StatusLinejs } from "../lib/StatusLine";

describe('StatusLine', function () {
    describe('#toJSON', function () {
        it('should return non-computed properties', function () {
            ext_unexpected_expect(new libStatusLine_StatusLinejs('HTTP/1.1 200 OK').toJSON(), 'to equal', {
                statusCode: 200,
                statusMessage: 'OK',
                protocolName: 'HTTP',
                protocolVersion: '1.1'
        ...
        it('should not include the keys that have undefined values', function () {
            ext_unexpected_expect(new libStatusLine_StatusLinejs('HTTP/1.1').toJSON(), 'not to have key', 'statusCode');
        });
    });
});
