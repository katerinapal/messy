...
import { isRegExp as isRegExp_isRegExp } from "./isRegExp";
import ext_iconvLite from "iconv-lite";
import ext_quotedPrintable from "quoted-printable";
import {     decodeChunkedTransferEncoding as decodeChunkedTransferEncoding_decodeChunkedTransferEncoding, } from "./decodeChunkedTransferEncoding";
...
Message.prototype.HeadersConstructor = Headers_Headers;
        ...
        if (!isDefined(this._unchunkedBody)) {
            if (isDefined(this._rawBody)) {
                ...
                if (transferEncoding && transferEncoding === 'chunked') {
                    try {
                        this._unchunkedBody = decodeChunkedTransferEncoding_decodeChunkedTransferEncoding(this._unchunkedBody);
                    } catch (e) {}
                }
                ...
                } else if (ext_iconvLite.encodingExists(charset) && !/^utf-?8$/i.test(charset)) {
                    this._unchunkedBody = ext_iconvLite.encode(this._unchunkedBody, charset);
                }
                ...
                if (contentTransferEncoding) {
                    ...
                    if (contentTransferEncoding === 'base64') {
                        ...
                        this._unchunkedBody = ext_quotedPrintable.encode(this._unchunkedBody);
                    }
                }
        ...
        if (this._parts) {
            ...
            if (contentTransferEncoding) {
                ...
                if (contentTransferEncoding === 'quoted-printable') {
                    ...
                    var qpDecodedBodyAsByteString = ext_quotedPrintable.decode(this._body);
            ...
            if (this.hasTextualContentType && contentTransferEncodingIsHonored && this._body && typeof this._body !== 'string') {
                var charset = this.charset;
                if (ext_iconvLite.encodingExists(charset)) {
                    this._body = ext_iconvLite.decode(this._body, charset);
                }
            }
...
function isNonBufferNonRegExpObject(obj) {
    return obj && typeof obj === 'object' && (typeof Buffer === 'undefined' || !Buffer.isBuffer(obj)) && !isRegExp_isRegExp(obj);
}
