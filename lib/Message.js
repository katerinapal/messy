...
import { isRegExpjs as isRegExp_isRegExpjsjs } from "./isRegExp";
import ext_iconvlite_iconvLite from "iconv-lite";
import ext_quotedprintable_quotedPrintable from "quoted-printable";
import {     decodeChunkedTransferEncodingjs as decodeChunkedTransferEncoding_decodeChunkedTransferEncodingjsjs, } from "./decodeChunkedTransferEncoding";
...
Message.prototype.HeadersConstructor = Headers_Headersjs;
        ...
        if (!isDefined(this._unchunkedBody)) {
            if (isDefined(this._rawBody)) {
                ...
                if (transferEncoding && transferEncoding === 'chunked') {
                    try {
                        this._unchunkedBody = decodeChunkedTransferEncoding_decodeChunkedTransferEncodingjsjs(this._unchunkedBody);
                    } catch (e) {}
                }
                ...
                } else if (ext_iconvlite_iconvLite.encodingExists(charset) && !/^utf-?8$/i.test(charset)) {
                    this._unchunkedBody = ext_iconvlite_iconvLite.encode(this._unchunkedBody, charset);
                }
                ...
                if (contentTransferEncoding) {
                    ...
                    if (contentTransferEncoding === 'base64') {
                        ...
                        this._unchunkedBody = ext_quotedprintable_quotedPrintable.encode(this._unchunkedBody);
                    }
                }
        ...
        if (this._parts) {
            ...
            if (contentTransferEncoding) {
                ...
                if (contentTransferEncoding === 'quoted-printable') {
                    ...
                    var qpDecodedBodyAsByteString = ext_quotedprintable_quotedPrintable.decode(this._body);
            ...
            if (this.hasTextualContentType && contentTransferEncodingIsHonored && this._body && typeof this._body !== 'string') {
                var charset = this.charset;
                if (ext_iconvlite_iconvLite.encodingExists(charset)) {
                    this._body = ext_iconvlite_iconvLite.decode(this._body, charset);
                }
            }
...
function isNonBufferNonRegExpObject(obj) {
    return obj && typeof obj === 'object' && (typeof Buffer === 'undefined' || !Buffer.isBuffer(obj)) && !isRegExp_isRegExpjsjs(obj);
}
...
export { Message_Message as Message };
