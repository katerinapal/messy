...
describe('RequestLine', function () {
    it('should add a leading slash to the url if not specified', function () {
        ext_unexpected_expect(new libRequestLine_RequestLinejs('GET foo').url, 'to equal', '/foo');
    });

    describe('#toString', function () {
        it('should omit an undefined protocol', function () {
            ext_unexpected_expect(new libRequestLine_RequestLinejs('GET /').toString(), 'to equal', 'GET /');
        });
    });

    describe('#toJSON', function () {
        it('should return non-computed properties', function () {
            ext_unexpected_expect(new libRequestLine_RequestLinejs('GET / HTTP/1.1').toJSON(), 'to equal', {
                method: 'GET',
                url: '/',
                protocolName: 'HTTP',
                protocolVersion: '1.1'
        ...
        it('should not include the keys that have undefined values', function () {
            ext_unexpected_expect(new libRequestLine_RequestLinejs('GET').toJSON(), 'not to have keys', ['path', 'protocolName', 'protocolVersion']);
        });
    });
});
