var HttpRequest_HttpRequest = HttpRequest;
import { Message as Message_Messagejs } from "./Message";
import { RequestLine as RequestLine_RequestLinejs } from "./RequestLine";
import ext_util_util from "util";
import ext_underscore__ from "underscore";

function HttpRequest(obj) {
    this.requestLine = new RequestLine_RequestLinejs();
...
HttpRequest.propertyNames = Message_Messagejs.propertyNames.concat(HttpRequest.metadataPropertyNames).concat(RequestLine_RequestLinejs.propertyNames).concat(['requestLine']);

ext_util_util.inherits(HttpRequest, Message_Messagejs);
...
HttpRequest.prototype.populateFromObject = function (obj) {
    ...
    if (typeof obj.url === 'string') {
        ...
        if (fragments.length > 0) {
            ...
            obj = ext_underscore__.extend({}, obj);
            obj.url = this.path;
        }
        if (fragments.length > 1) {
    ...
    this.requestLine.populateFromObject(ext_underscore__.omit(obj, 'url'));
    return this;
};
...
HttpRequest.prototype.populateFromString = function (str) {
    ...
    if (matchRequestLine) {
        Message_Messagejs.prototype.populateFromString.call(this, str.substr(matchRequestLine[0].length));
...
RequestLine_RequestLinejs.propertyNames.forEach(function (requestLinePropertyName) {
...
HttpRequest.prototype.toJSON = function () {
    return ext_underscore__.extend(Message_Messagejs.prototype.toJSON.call(this), this.requestLine.toJSON());
};

export { HttpRequest_HttpRequest as HttpRequest };
