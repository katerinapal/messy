var Headers_Headers = Headers;
import { foldHeaderLinejs as foldHeaderLine_foldHeaderLinejsjs } from "./foldHeaderLine";
import { formatHeaderName as formatHeaderName_formatHeaderNamejs } from "./formatHeaderName";
import { isRegExpjs as isRegExp_isRegExpjsjs } from "./isRegExp";
import ext_rfc2231_rfc2231 from "rfc2231";
    ...
    if (typeof stringOrArrayOrRegExp === 'undefined') {
        ...
        if (Array.isArray(stringOrArrayOrRegExp)) {
            return stringOrArrayOrRegExp.every(function (expectedValue) {
                if (isRegExp_isRegExpjsjs(expectedValue)) {
...
Headers.prototype.parameter = function (headerName, attributeName, attributeValue) {
    ...
    if (headerValue) {
        ...
        if (usesRfc2231 && !rfc2231DisabledForThisHeader) {
            parameters = ext_rfc2231_rfc2231.unfoldAndDecodeParameters(parameters);
        }
        if (attributeName) {
            if (attributeValue) {
                ...
                if (!rfc2231DisabledForThisHeader) {
                    parameters = ext_rfc2231_rfc2231.encodeAndFoldParameters(parameters);
                }
...
Headers.prototype.toString = function (maxLineLength) {
    ...
    lowerCaseHeaderNames.forEach(function (lowerCaseHeaderName) {
        this.valuesByName[lowerCaseHeaderName].forEach(function (value) {
            result += formatHeaderName_formatHeaderNamejs(lowerCaseHeaderName) + ': ' + foldHeaderLine_foldHeaderLinejsjs(this.serializeHeaderValue(value), maxLineLength, maxLineLength - lowerCaseHeaderName.length - 2) + '\r\n';
        }, this);
    }, this);
    return result;
};
...
Headers.prototype.toCanonicalObject = function () {
    ...
    this.getNames().forEach(function (lowerCaseHeaderName) {
        canonicalObject[formatHeaderName_formatHeaderNamejs(lowerCaseHeaderName)] = this.getAll(lowerCaseHeaderName);
    }, this);
...
Headers.prototype.toJSON = function () {
    ...
    this.getNames().forEach(function (lowerCaseHeaderName) {
        ...
        if (values.length === 1) {
            obj[formatHeaderName_formatHeaderNamejs(lowerCaseHeaderName)] = this.get(lowerCaseHeaderName);
        } else {
            obj[formatHeaderName_formatHeaderNamejs(lowerCaseHeaderName)] = this.getAll(lowerCaseHeaderName);
        }
    }, this);
    return obj;
};

export { Headers_Headers as Headers };
