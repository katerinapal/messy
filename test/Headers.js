import ext_unexpected_unexpected from "unexpected";
import { Headers as libHeaders_Headersjs } from "../lib/Headers";
import ext_unexpectedmessy_unexpectedmessy from "unexpected-messy";

describe('Headers', function () {
    var expect = ext_unexpected_unexpected.clone();

    // Not published yet
    try {
        expect.installPlugin(ext_unexpectedmessy_unexpectedmessy);
    } catch (e) {}

    it('should accept a string', function () {
        var headers = new libHeaders_Headersjs('Subject: hey, dude!');
    ...
    it('should fold the lines when serializing', function () {
        var headers = new libHeaders_Headersjs({subject: 'hey there, dude!'});
    ...
    describe('#remove', function () {
        it('should remove all header values for the given header when only passed one argument', function () {
            var headers = new libHeaders_Headersjs({foo: ['bla', 'bar'], quux: 'baz'});
            headers.remove('foo');
            expect(headers, 'to equal', new libHeaders_Headersjs({quux: 'baz'}));
        });

        it('should remove a single header value', function () {
            var headers = new libHeaders_Headersjs({foo: 'bar', quux: 'baz'});
            headers.remove('foo', 'bar');
            expect(headers, 'to equal', new libHeaders_Headersjs({quux: 'baz'}));
        });

        it('should remove one out of multiple values', function () {
            var headers = new libHeaders_Headersjs({foo: ['bar', 'bla'], quux: 'baz'});
            headers.remove('foo', 'bar');
            expect(headers, 'to equal', new libHeaders_Headersjs({foo: 'bla', quux: 'baz'}));
        });

        it('should remove multiple values, leaving one', function () {
            var headers = new libHeaders_Headersjs({foo: ['bar', 'bla', 'hey'], quux: 'baz'});
            headers.remove('foo', ['bar', 'hey']);
            expect(headers, 'to equal', new libHeaders_Headersjs({foo: 'bla', quux: 'baz'}));
        });

        it('should remove multiple values, leaving none', function () {
            var headers = new libHeaders_Headersjs({foo: ['bla', 'hey'], quux: 'baz'});
        ...
        it('should remove all header values found in object', function () {
            ...
            expect(headers, 'to equal', new libHeaders_Headersjs({foo: 'bla'}));
        });
        ...
        it('should return the number of removed values when removing all values of a header', function () {
            expect(new libHeaders_Headersjs({foo: ['bla', 'hey'], quux: 'baz'}).remove('foo'), 'to equal', 2);
        });

        it('should return the number of removed values when removing one out of two', function () {
            expect(new libHeaders_Headersjs({foo: ['bla', 'hey'], quux: 'baz'}).remove('foo', 'hey'), 'to equal', 1);
        });

        it('should return the number of removed values when removing two out of two', function () {
            expect(new libHeaders_Headersjs({foo: ['bla', 'hey'], quux: 'baz'}).remove('foo', ['bla', 'hey']), 'to equal', 2);
        });

        it('should return 0 when the attempting to remove a single value that was not found', function () {
            expect(new libHeaders_Headersjs({foo: 'hey', quux: 'baz'}).remove('foo', 'dah'), 'to equal', 0);
        });

        it('should return 0 when the attempting to remove multiple value that were not found', function () {
            expect(new libHeaders_Headersjs({foo: 'hey', quux: 'baz'}).remove('foo', ['dah', 'bla']), 'to equal', 0);
        });
    });

    describe('#toCanonicalObject', function () {
        expect(new libHeaders_Headersjs({foo: 'hey', quux: 'baz'}).toCanonicalObject(), 'to equal', {
            Foo: ['hey'],
            Quux: ['baz']
        });
    });
    ...
    describe('#set', function () {
        it('should preserve existing values when adding a string value', function () {
            var headers = new libHeaders_Headersjs({foo: 'bar'});
        ...
        describe('when passed an object', function () {
            it('should act as a shorthand for calling set with each (key, value)', function () {
                var headers = new libHeaders_Headersjs({foo: 'bar', baz: 'quux'});
                headers.set({
                    foo: [ 'bar2' ],
                    baz: 'quux2',
                    yadda: 'blah'
    ...
    describe('#parameter()', function () {
        describe('when called with just a header name', function () {
            it('should return a hash of attributes when the header has attributes', function () {
                expect(new libHeaders_Headersjs('Foo: bar; quux=baz').parameter('Foo'), 'to equal', {quux: 'baz'});
            });
            ...
            it('should return an empty hash when the header has no attributes', function () {
                expect(new libHeaders_Headersjs('Foo: bar').parameter('Foo'), 'to equal', {});
            });

            it('should return undefined when the header does not exist', function () {
                expect(new libHeaders_Headersjs('Foo: bar').parameter('Quux'), 'to equal', undefined);
            });

            it('should decode rfc2231-encoded attributes', function () {
                expect(new libHeaders_Headersjs(
        ...
        describe('when called with a header name and an attribute name', function () {
            it('should return the attribute value', function () {
                expect(new libHeaders_Headersjs('Foo: hey').parameter('Foo'), 'to equal', {});
            });

            it('should return undefined when the header has no attributes', function () {
                expect(new libHeaders_Headersjs('Foo: hey').parameter('Foo', 'bar'), 'to equal', undefined);
            });

            it('should return undefined when the header has attributes, but not the one being asked for', function () {
                expect(new libHeaders_Headersjs('Foo: hey; quux=blah').parameter('Foo', 'bar'), 'to equal', undefined);
            });

            it('should return undefined when the header is not there', function () {
                expect(new libHeaders_Headersjs('Foo: bar').parameter('Bar', 'quux'), 'to equal', undefined);
            });
        });
        ...
        describe('when called with a header name, an attribute name, and an attribute value', function () {
            it('should define the attribute if it does not exist', function () {
                var headers = new libHeaders_Headersjs('Foo: hey');
            ...
            it('should update the attribute if it already exists', function () {
                var headers = new libHeaders_Headersjs('Foo: hey; blah=quux');
                headers.parameter('Foo', 'blah', 'baz');
                expect(headers.toString(), 'to equal', 'Foo: hey; blah=baz\r\n');
            });

            it('should transparently encode non-ASCII attribute values using rfc2231', function () {
                var headers = new libHeaders_Headersjs('Content-Type: text/plain');
