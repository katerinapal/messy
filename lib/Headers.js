    ...
    if (typeof stringOrArrayOrRegExp === 'undefined') {
        ...
        if (Array.isArray(stringOrArrayOrRegExp)) {
            return stringOrArrayOrRegExp.every(function (expectedValue) {
                if (isRegExp_isRegExp(expectedValue)) {
...
Headers.prototype.toString = function (maxLineLength) {
    ...
    lowerCaseHeaderNames.forEach(function (lowerCaseHeaderName) {
        this.valuesByName[lowerCaseHeaderName].forEach(function (value) {
            result += formatHeaderName_formatHeaderName(lowerCaseHeaderName) + ': ' + foldHeaderLine_foldHeaderLine(this.serializeHeaderValue(value), maxLineLength, maxLineLength - lowerCaseHeaderName.length - 2) + '\r\n';
        }, this);
    }, this);
    return result;
};
...
Headers.prototype.toCanonicalObject = function () {
    ...
    this.getNames().forEach(function (lowerCaseHeaderName) {
        canonicalObject[formatHeaderName_formatHeaderName(lowerCaseHeaderName)] = this.getAll(lowerCaseHeaderName);
    }, this);
...
Headers.prototype.toJSON = function () {
    ...
    this.getNames().forEach(function (lowerCaseHeaderName) {
        ...
        if (values.length === 1) {
            obj[formatHeaderName_formatHeaderName(lowerCaseHeaderName)] = this.get(lowerCaseHeaderName);
        } else {
            obj[formatHeaderName_formatHeaderName(lowerCaseHeaderName)] = this.getAll(lowerCaseHeaderName);
        }
    }, this);
    return obj;
};

export { mod_Headers as Headers };
