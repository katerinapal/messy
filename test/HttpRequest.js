...
describe('HttpRequest', function () {
    ...
    it('should accept encrypted as a parameter to the constructor', function () {
        expect(new libHttpRequest_HttpRequestjs({encrypted: true}), 'to have properties', {encrypted: true});
    });

    it('should accept the request line as an option to the constructor', function () {
        expect(new libHttpRequest_HttpRequestjs({requestLine: 'GET /foo HTTP/1.1'}), 'to have properties', {
            method: 'GET',
            url: '/foo',
            protocol: 'HTTP/1.1'
        });
    ...
    it('should parse a partial request line', function () {
        expect(new libHttpRequest_HttpRequestjs('GET /foo'), 'to have properties', {
            method: 'GET',
            url: '/foo'
        });
    });
    ...
    it('should only include CRLFCRLF when there are no headers', function () {
        expect(new libHttpRequest_HttpRequestjs({
    ...
    it('should make the protocol version available as a getter', function () {
        expect(new libHttpRequest_HttpRequestjs('GET /foo HTTP/1.1').protocolVersion, 'to equal', '1.1');
    });

    it('should make the protocol name available as a getter', function () {
        expect(new libHttpRequest_HttpRequestjs('GET /foo HTTP/1.1').protocolName, 'to equal', 'HTTP');
    });

    it('should make the components of the request url available as individual getters', function () {
        expect(new libHttpRequest_HttpRequestjs('GET /foo?foo=bar HTTP/1.1'), 'to have properties', {
            path: '/foo',
            search: '?foo=bar',
            query: 'foo=bar'
        });
    ...
    describe('without a verb in the request line', function () {
        it('should assume GET', function () {
            expect(new libHttpRequest_HttpRequestjs('/foo'), 'to satisfy', {
                method: 'GET',
                path: '/foo'
            });
        });
    ...
    it('should accept host and port as options', function () {
        expect(new libHttpRequest_HttpRequestjs({ host: 'foo.com', port: 987 }), 'to have properties', {
            host: 'foo.com',
            port: 987
        });
    });
    ...
    it('should leave host and port as undefined when not given', function () {
        expect(new libHttpRequest_HttpRequestjs('GET /').port, 'to be undefined');
    });

    describe('#url', function () {
        it('should include the schema, host, port, path, and search if available', function () {
            expect(new libHttpRequest_HttpRequestjs('GET https://localhost:3000/foo?bar=quux').url, 'to equal', 'https://localhost:3000/foo?bar=quux');
        });

        it('should include the default http port explicitly', function () {
            expect(new libHttpRequest_HttpRequestjs('GET http://localhost/').url, 'to equal', 'http://localhost/');
        });

        it('should include the default https port explicitly', function () {
            expect(new libHttpRequest_HttpRequestjs('GET https://localhost/').url, 'to equal', 'https://localhost/');
        });

        it('should include the username and password if available', function () {
            expect(new libHttpRequest_HttpRequestjs('GET http://foo:bar@localhost:3000/').url, 'to equal', 'http://foo:bar@localhost:3000/');
        });
    ...
    describe('with a url passed in the request line', function () {
        ...
        it('should set the host property', function () {
            expect(new libHttpRequest_HttpRequestjs('GET http://foo.com/').host, 'to equal', 'foo.com');
        });

        it('should set the port property when explicitly given', function () {
            expect(new libHttpRequest_HttpRequestjs('GET http://foo.com:987/').port, 'to equal', 987);
        });

        it('should set the port property to 80 when http', function () {
            expect(new libHttpRequest_HttpRequestjs('GET http://foo.com/').port, 'to equal', 80);
        });

        it('should set the port property to 443 when https', function () {
            expect(new libHttpRequest_HttpRequestjs('GET https://foo.com/').port, 'to equal', 443);
        });

        it('should set the path', function () {
            expect(new libHttpRequest_HttpRequestjs('GET http://foo.com/heythere/you').path, 'to equal', '/heythere/you');
        });

        it('should set the Host header and include the port if given', function () {
            expect(new libHttpRequest_HttpRequestjs('GET http://foo.com:987/').headers.get('Host'), 'to equal', 'foo.com:987');
        });

        it('should not overwrite an explicit Host header', function () {
            expect(new libHttpRequest_HttpRequestjs('GET http://foo.com/\r\nHost: bar.com').headers.getAll('Host'), 'to equal', [ 'bar.com' ]);
        });
        ...
        it('should set the "encrypted" property if the protocol is https', function () {
            expect(new libHttpRequest_HttpRequestjs('GET https://foo.com/'), 'to satisfy', { encrypted: true });
        });

        it('should not set the "encrypted" property if the protocol is http', function () {
            expect(new libHttpRequest_HttpRequestjs('GET http://foo.com/'), 'to satisfy', { encrypted: expect.it('to be falsy') });
        });
    ...
    describe('with a url passed in an options object', function () {
        ...
        it('should set the host property', function () {
            expect(new libHttpRequest_HttpRequestjs({ url: 'GET http://foo.com/' }).host, 'to equal', 'foo.com');
        });

        it('should set the port property when explicitly given', function () {
            expect(new libHttpRequest_HttpRequestjs({ url: 'GET http://foo.com:987/' }).port, 'to equal', 987);
        });

        it('should set the port property to 80 when http', function () {
            expect(new libHttpRequest_HttpRequestjs({ url: 'GET http://foo.com/' }).port, 'to equal', 80);
        });

        it('should set the port property to 443 when https', function () {
            expect(new libHttpRequest_HttpRequestjs({ url: 'GET https://foo.com/' }).port, 'to equal', 443);
        });

        it('should set the path', function () {
            expect(new libHttpRequest_HttpRequestjs({ url: 'GET http://foo.com/heythere/you' }).path, 'to equal', '/heythere/you');
        });
        ...
        it('should set the "encrypted" property if the protocol is https', function () {
            expect(new libHttpRequest_HttpRequestjs({ url: 'GET https://foo.com/' }), 'to satisfy', { encrypted: true });
        });

        it('should not set the "encrypted" property if the protocol is http', function () {
            expect(new libHttpRequest_HttpRequestjs({ url: 'GET http://foo.com/' }), 'to satisfy', { encrypted: expect.it('to be falsy') });
        });
    ...
    describe('#toJSON', function () {
        it('should include the messy.Message properties and put the requestLine properties at the top level', function () {
            expect(new libHttpRequest_HttpRequestjs('GET / HTTP/1.1\r\nFoo: bar\r\n\r\nblabla').toJSON(), 'to equal', {
                method: 'GET',
                url: '/',
                protocolName: 'HTTP',
                protocolVersion: '1.1',
